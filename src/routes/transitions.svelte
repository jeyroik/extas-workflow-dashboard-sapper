...
<script>
    ...
    let filters  = [
        ...
        },
        {
            "field": "state_from",
            "sub": "title",
            "value": extractFilter('state_from', jsonRpcFilter)
        },
        {
            "field": "state_to",
            "sub": "title",
            "value": extractFilter('state_to', jsonRpcFilter)
        },
        {
            "field": "schema",
            "sub": "title",
    ...
    function reloadItems(items, value){
        entities = items
    }
</script>
...
<table>
    <thead>
        ...
        <Filters bind:filters={filters} bind:items={entities} afterFilterHook={reloadItems}/>
    </thead>
    <tbody>
    {#each entities as item}
        <tr>
            ...
            <td style="min-width: 150px">
                <button class="btn btn-danger" title="Удалить переход" on:click={deleteEntity(item, 'transition', 'Переход', item => false, 'Невозможно удалить переход', afterDelete)}>
                ...
                <button class="btn btn-primary" title="Сохранить изменения" on:click={save(item, 'transition', 'Переход')}>
