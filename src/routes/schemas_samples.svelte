<script context="module">
    export function preload({ params, query }) {
        ...
        return this.fetch('process.JSON_RPC__ENDPOINT', {
            method: 'POST',
            body: JSON.stringify({method: "workflow.schema.sample.index"})
...
<svelte:head>
    <title>Шаблоны схем</title>
</svelte:head>

<h2>Шаблоны схем - именованные объекты, использующиеся для создания новых схем.</h2>
...
<Create entity="schemas_samples" text="Создать шаблон схемы"/>
    ...
    {#each entities as item}
        <tr>
            ...
            <td style="min-width: 200px">
                <button class="btn btn-danger" title="Удалить" on:click={deleteEntity(item, 'schema.sample', 'Шаблон схемы', e => false, '', afterDelete)}>
                ...
                <button class="btn btn-primary" title="Сохранить изменения" on:click={save(item, 'schema.sample', 'Шаблон схемы')} value="">
